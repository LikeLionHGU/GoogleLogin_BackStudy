...
public class LoginController {
    ...
    private final String REDIRECT_URI = "http://localhost:8080/redirect.html"; // 프론트와 동일하게 설정
    ...
    public ResponseEntity<?> googleLogin(@RequestParam String authorizationCode) {
        System.out.println("LoginController - authorization_code : " + authorizationCode);
        ...
        if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
            String accessToken = (String) response.getBody().get("access_token"); // accessToken 추출하는 부분
            System.out.println("LoginController - access_token >> " + accessToken);
